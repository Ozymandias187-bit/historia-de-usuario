/**
 * AutoDrive - Lógica de Gestión de Concesionario
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (tituloDash) tituloDash.textContent = 'Bienvenido, ' + MOCK_DATA.usuario.nombre;
    const mascaraCuenta = document.getElementById('account-mask');
    if (mascaraCuenta) mascaraCuenta.textContent = 'Sede: ' + MOCK_DATA.cuenta.numeroEnmascarado;

    if (toggleVisBtn && balanceEl) {
        let saldoVisible = true;
        const textoSaldoReal = 'USD ' + MOCK_DATA.cuenta.saldoDisponible.toLocaleString();
        ...
        for (let i = 0; i < MOCK_DATA.transacciones.length; i++) {
            ...
            const itemHTML = `
                <article class="tx-item" style="border: 1px solid #ccc; padding: 10px; margin-bottom: 10px; border-radius: 8px; cursor: pointer;">
                    ...
                    <strong style="${colorClase}">${signo} $${tx.monto.toLocaleString()}</strong>
                </article>
            `;
    ...
    if (paginaActual === 'perfil') {
        ...
        if (formPerfil) {
            ...
            formPerfil.addEventListener('submit', (evento) => {
                ...
                if (nuevoNombre) {
                    MOCK_DATA.usuario.nombre = nuevoNombre;
        ...
        if (btnEliminar) {
            btnEliminar.addEventListener('click', () => {
                if (confirm('¿ESTÁ SEGURO? Esta acción eliminará su cuenta de comprador permanentemente.')) {
                    localStorage.removeItem('usuarioActivoAutoDrive');
